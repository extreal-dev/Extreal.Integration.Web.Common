...
class VideoPlayerAdopter {
    ...
    public adopt = () => {
        ...
        addAction(this.withPrefix("ReleaseManagedResources"), (instanceId) =>
            this.getVideoPlayer(instanceId).releaseManagedResources(),
        );
        addAction(this.withPrefix("Clear"), (instanceId) => this.getVideoPlayer(instanceId).clear());
        addAction(this.withPrefix("Prepare"), (filePath, instanceId) =>
            this.getVideoPlayer(instanceId).prepare(filePath),
        );
        ...
        addAction(this.withPrefix("UpdateTexture"), (textureId, instanceId) =>
            this.getVideoPlayer(instanceId).updateTexture(Number.parseInt(textureId)),
        );
        addFunction(this.withPrefix("GetLength"), (instanceId) => this.getVideoPlayer(instanceId).getLength());
        addAction(this.withPrefix("SetTime"), (time, instanceId) =>
            this.getVideoPlayer(instanceId).setTime(Number.parseFloat(time)),
        );
        addAction(this.withPrefix("SetAudioVolume"), (volume, instanceId) =>
            this.getVideoPlayer(instanceId).setAudioVolume(Number.parseFloat(volume)),
        );
    };
